...
// Each thread processes one or more pairs of amplitudes (via grid-stride loop):
// one where the target bit is 0, and the corresponding one where the target bit
// is 1.
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    while (thread_id < num_pairs) {
        ...
        // a' = gate[0]*a + gate[1]*b
        let new_a = cmul(params.gate_re0, params.gate_im0, a_re, a_im)
                  + cmul(params.gate_re1, params.gate_im1, b_re, b_im);

        // b' = gate[2]*a + gate[3]*b
        let new_b = cmul(params.gate_re2, params.gate_im2, a_re, a_im)
                  + cmul(params.gate_re3, params.gate_im3, b_re, b_im);

        // Write back in-place.
        state[off0] = new_a.x;
        state[off0 + 1u] = new_a.y;
        state[off1] = new_b.x;
        state[off1 + 1u] = new_b.y;

...
impl GpuDevice {
    ...
    /// - Must have at least 1 GB storage buffer binding size
        ...
        adapters
            ...
            .filter(|a| {
                ...
                score.0 > 0 // discrete or integrated
                    ...
                    && score.3 >= (1u32 << 30) // at least 1 GB storage buffers
            })
    ...
    /// GPU's max buffer size and the bytes required per amplitude.
    ///
    /// For f32 complex (the Phase 1 precision), `bytes_per_amplitude` is 8
    /// (two f32 values per complex number).
    ///
    /// Returns `n` such that `2^n * bytes_per_amplitude <= max_buffer_size`.
    #[must_use]
    pub fn max_qubits(&self, bytes_per_amplitude: u64) -> u32 {
        let max_amplitudes = self.max_buffer_size / bytes_per_amplitude;
        if max_amplitudes == 0 {
            return 0;
        }
        // 63 - leading_zeros gives floor(log2(n)) for n > 0
        #[allow(clippy::cast_possible_truncation)]
        let result = 63 - max_amplitudes.leading_zeros();
        result
    }
}
